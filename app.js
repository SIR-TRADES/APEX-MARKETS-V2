...
const investmentPackages = [
  { name: 'Starter', amount: 15000 },
  { name: 'Growth', amount: 50000 },
  { name: 'Builder', amount: 250000 },
  { name: 'Momentum', amount: 1000000 },
  { name: 'Professional', amount: 5000000 },
  { name: 'Apex', amount: 20000000 }
];
...
function renderOverview(){
  ...
  <div class="package-section"><div class="welcome-row"><div><h3>Choose an investment package</h3><p>Six fixed packages, each running for 14 days at 20% daily.</p></div><div class="date-badge">UGX 15,000 – UGX 20,000,000</div></div><div class="package-grid">${investmentPackages.map((investmentPackage, index) => `<article class="content-card package-card"><div class="package-head"><span class="step-number">0${index + 1}</span><span class="package-rate">20% / day</span></div><h3>${investmentPackage.name}</h3><strong class="package-amount">${money(investmentPackage.amount)}</strong><p>14-day investment package</p><button class="action-button deposit" data-action="deposit" data-package-amount="${investmentPackage.amount}">Choose package</button></article>`).join('')}</div></div>`;
  bindDynamicEvents();
}
...
function renderView(view){
  ...
  if(view === 'activity'){ viewContent.innerHTML = `<div class="welcome-row"><div><h3>How to invest</h3><p>Follow these simple steps to put your capital to work.</p></div></div><div class="investment-guide"><article class="content-card investment-step"><span class="step-number">01</span><div><h3>Choose your amount</h3><p>Choose one of the investment packages on your Overview page, starting from UGX 15,000.</p></div></article><article class="content-card investment-step"><span class="step-number">02</span><div><h3>Send your deposit</h3><p>Choose your package, select your mobile-money provider, and send the payment to the displayed number.</p></div></article><article class="content-card investment-step"><span class="step-number">03</span><div><h3>Wait for confirmation</h3><p>Submit your payment details. Our team reviews the deposit and confirms it manually.</p></div></article><article class="content-card investment-step"><span class="step-number">04</span><div><h3>Track your investment</h3><p>Once confirmed, your balance and active 14-day plan appear on your Overview page.</p></div></article></div>`; }
...
function openModal(type, selectedAmount){
  const isDeposit = type === 'deposit';
  const modal = document.createElement('div'); modal.className = 'modal-backdrop'; modal.id = 'moneyModal';
  const packageOptions = investmentPackages.map(investmentPackage => `<option value="${investmentPackage.amount}" ${Number(selectedAmount) === investmentPackage.amount ? 'selected' : ''}>${investmentPackage.name} · ${money(investmentPackage.amount)} · 14 days</option>`).join('');
  modal.innerHTML = `<div class="modal"><h3>${isDeposit ? 'Choose an investment package' : 'Request a withdrawal'}</h3><p>${isDeposit ? 'Every package runs for 14 days and targets 20% per day. Select one package to continue.' : 'Withdraw available funds to your mobile money account.'}</p>${isDeposit ? `<label for="package">Investment package</label><select id="package">${packageOptions}</select><label for="provider">Mobile money provider</label><select id="provider"><option>MTN Mobile Money</option><option>Airtel Money</option></select><div class="payment-destination"><strong>Send payment to</strong><b>+256 758 760 840</b><small>Use the selected mobile-money network, then submit your amount below.</small></div><input id="amount" type="hidden" value="${selectedAmount || investmentPackages[0].amount}"><div class="limit-note">Six fixed packages · UGX 15,000 to UGX 20,000,000 · 14 days · 20% daily</div>` : '<label for="withdrawProvider">Send to</label><select id="withdrawProvider"><option>MTN Mobile Money · 0772 123 456</option><option>Airtel Money · 0701 555 024</option></select><label for="amount">Amount (UGX)</label><input id="amount" type="number" min="1000" max="20000000" placeholder="100,000"><div class="limit-note">Available to withdraw: UGX 1,250,000</div>'}<div class="modal-actions"><button class="cancel-button" id="cancelModal">Cancel</button><button class="confirm-button" id="confirmModal">${isDeposit ? 'Submit package' : 'Request withdrawal'}</button></div></div>`;
  document.body.appendChild(modal); document.getElementById('cancelModal').onclick = () => modal.remove(); document.getElementById('confirmModal').onclick = () => { const amount = Number(document.getElementById('amount').value); const isValidPackage = investmentPackages.some(investmentPackage => investmentPackage.amount === amount); if(isDeposit && !isValidPackage){ showToast('Choose one of the six available investment packages.'); return; } if(!amount){ showToast('Enter an amount to continue.'); return; } pendingRequests.unshift({ name: currentUser.name, email: currentUser.email, type: isDeposit ? 'deposit' : 'withdrawal', amount, submitted: 'Just now', status: 'Pending' }); persistRequests(); modal.remove(); showToast(isDeposit ? 'Deposit sent to admin for confirmation.' : 'Withdrawal sent to admin for confirmation.'); };
  if(isDeposit){ document.getElementById('package').onchange = event => { document.getElementById('amount').value = event.target.value; }; }
}
function bindDynamicEvents(){ document.querySelectorAll('[data-action]').forEach(button => button.onclick = () => openModal(button.dataset.action, button.dataset.packageAmount)); document.querySelectorAll('[data-view-link]').forEach(link => link.onclick = event => { event.preventDefault(); renderView(link.dataset.viewLink); }); document.querySelectorAll('[data-market-symbol]').forEach(button => button.onclick = () => openMarketChat(button.dataset.marketSymbol)); document.querySelectorAll('.filter-chip').forEach(button => button.onclick = () => { document.querySelectorAll('.filter-chip').forEach(chip => chip.classList.remove('active')); button.classList.add('active'); const category = button.textContent; document.querySelectorAll('.market-tile').forEach(tile => { const market = marketData.find(item => item.symbol === tile.dataset.marketSymbol); tile.hidden = category !== 'All' && market.category !== category; }); }); }
